...
The `flights` data frame has `r nrow(flights)` rows. Each row represents an observation.
...
A negative `dep_delay` means that the flight has departed early.
...
The plane tail number is N384HA.
...
Take the flights data frame and then filter for flights whose destination is in RDU and/or GSO with an arrival delay or departure delay less than 0.
...
February had the fewest number of flights. There were 24,951 flights that month.
...
November 27 had the largest number of flights with 1014.
...
The proportion of flights in July is 0.0871.
...
flights |>
  ...
  mutate(RDU_EWR = 1482/119353) |>
  mutate(RDU_JFK = 3100/108179) |>
  mutate(RDU_LGA = 3581/101081) 
...
LGA has the shortest median departure delay.
